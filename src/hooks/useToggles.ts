...
export const isValidState = (state: TogglesState) =>
  !(state.good && state.fast && state.cheap);

export const adjustState = (draftState: TogglesState, key: ToggleKey) => {
  // find previous key to flip
  const keyToUpdate =
    key === "good" ? "cheap" : key === "fast" ? "good" : "fast";
...
const togglesReducer = (prevState: TogglesState, action: ToggleAction) => {
  ...
  return isValidState(draftState)
    ? draftState
    : adjustState(draftState, toggleKey);
};
